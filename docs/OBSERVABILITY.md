...
Historical-analysis `queued`, `running`, `succeeded`, `failed`, and `cancelled` states describe request/lifecycle persistence only; they do not indicate dataset coverage, simulation execution, report availability, or provider work. The API records safe lifecycle events and never logs candle payloads, provider details, report bodies, or user identifiers.
