...
`historical_analysis_reports` stores one immutable successful result per run and dataset. It contains UUID foreign keys to the run, owner, and dataset; unique `run_id` and `dataset_id`; lowercase SHA-256 result, dataset, and strategy fingerprints; immutable strategy and exit-reason-count JSONB snapshots copied at publication; engine, assumption, and calculation versions; schema-versioned JSONB market, preset, coverage, assumptions, and safety-disclosure snapshots; UTC analysis bounds; nonnegative signal/trade/equity counts; exact-decimal summary metrics; and nullable win-rate/profit-factor values paired with an explicit undefined reason. Exit-reason counts are computed from the complete immutable result before publication and must sum to `trade_count`; they are not derived from paginated API rows. Run and owner deletion cascades; dataset deletion is restricted. Fingerprint, metric, range, count, and undefined-value checks enforce the immutable report contract.

`historical_analysis_trades` stores the complete immutable engine trade output for a report, including signal/position direction, candle identity/revisions/times, raw/fill prices, persisted exit reason, exit price basis, exit-rule snapshot, holding count, fee/slippage, returns, PnL, equity before/after, and outcome. Legacy rows are backfilled with the fixed-horizon six-candle, confirmed-close exit metadata. It has CASCADE deletion from the report, `ON DELETE RESTRICT` candle references, unique `(report_id, sequence)`, and report/sequence plus candle-reference indexes. `historical_analysis_equity_points` stores one ordered immutable point per visible analysis candle with candle identity/revision/times, equity, drawdown, position state, and optional active trade sequence; it has the same report cascade/candle restrict pattern and unique `(report_id, sequence)`.
...
Schema contents were read from the SQLAlchemy registry and Alembic chain. Local database connection and migration application were exercised by the maintainer-requested full-stack startup pass; rollback, schema-diff, and production migration behavior remain unverified.
