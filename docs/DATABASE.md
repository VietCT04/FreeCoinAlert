...
| Historical analysis | `historical_analysis_runs`, `historical_analysis_datasets`, `historical_analysis_dataset_candles`, `historical_analysis_reports`, `historical_analysis_trades`, `historical_analysis_equity_points` | owner-scoped request/lifecycle state, immutable canonical candle snapshots, and immutable report series |
...
`candle_backfill_checkpoints` stores one resumable public-archive unit per archive key, including market/timeframe, monthly or daily period, verified checksum, processing status, attempt count, next incomplete open time, and safe failure category. `market_candle_coverage` stores the derived target/available/contiguous range, missing-range count, coverage percentage, status, and verification/success timestamps for each market/timeframe. `provider_rest_rate_state` is the shared Binance Spot REST reservation/observation/block row used by catalog and reconciliation consumers; it contains no credentials or provider payloads. The dedicated candle backfill advisory lock is held on its database connection only while one worker owns planning/import work.
...
`historical_analysis_runs` stores one authenticated user's bounded historical-analysis request and its safe lifecycle metadata. It contains a UUID primary key, the owner, `supported_market_id`, `signal_preset_id`, a UUID `idempotency_key`, immutable exchange/market/symbol, preset, and strategy snapshots, server-resolved `calculation_version_snapshot`, strategy/simulation/assumption versions, UTC `analysis_start`/`analysis_end`, progress stage/percent, attempt and availability fields, cancellation/worker timestamps, safe `failure_code`, and created/updated timestamps. The strategy snapshot is canonical JSON with a SHA-256 fingerprint and is never edited after creation. The request snapshot preserves the market, fixed entry preset, position direction, and normalized exit rules selected at creation; later dataset, simulation, report, and frontend work does not alter this row.
...
The status is one of `queued`, `running`, `succeeded`, `failed`, or `cancelled`. Lifecycle checks require queued rows to have no terminal timestamps, running rows to have `started_at` and no terminal timestamp, succeeded rows to have `completed_at`, failed rows to have `failed_at` and a failure code, and cancelled rows to have `cancelled_at`. `analysis_end > analysis_start`, progress is 0 through 100, attempts are nonnegative and bounded by `max_attempts` (default 3), and the strategy fingerprint is lowercase SHA-256. The unique `(user_id, idempotency_key)` constraint makes create replay owner-scoped and prevents a key from creating a second logical run; service-level replay comparison also includes the normalized strategy fingerprint. Owner listing uses `(user_id, created_at DESC, id DESC)`; future queued work uses a partial `(available_at, created_at)` index; active-owner limits use a partial user/status index.

The user foreign key cascades on account deletion. Supported-market and signal-preset foreign keys use `ON DELETE RESTRICT` so a run retains the references required by its immutable request snapshot. Run creation validates configuration, strategy rules, range, ownership, and limits, reads the exact canonical coverage boundary, and then inserts the queued row in one transaction; it never calls a provider or waits for the background backfill worker. Existing rows are backfilled as `legacy_fixed_horizon_v1` snapshots with their original signal direction and six-candle exit; the migration computes fingerprints with Python `hashlib` and does not require a database hashing extension. Cancellation locks the owner row, immediately transitions queued work to cancelled or records `cancellation_requested_at` for running work, and commits the lifecycle change. The worker may requeue stale running claims while preserving attempt history; queued and running work is never eligible for cleanup.

## Historical Analysis Dataset Domain

`historical_analysis_datasets` stores one canonical coverage manifest per run. Its `status` is `ready`, `failed`, or `stale`; a failed row contains one safe preparation failure category and no snapshot candle rows. A ready row records the fixed timeframe, user-visible analysis range, exact warm-up boundary/count, analysis/total counts, first/last stored boundaries, lowercase SHA-256 `manifest_fingerprint`, and preparation/staleness timestamps. The run FK cascades, while market and preset FKs restrict deletion. Counts are nonnegative, `total_candle_count = warmup_candle_count + analysis_candle_count`, and the database bound is derived by timeframe plus the stored warm-up: at most `17,520 + required_warmup_candles` for `1h` and `4,380 + required_warmup_candles` for `4h`, with the current maximum warm-up of 200. Application preparation also rejects any request beyond the server's 730-day range before querying rows. Ready rows have no failure or stale timestamp; stale rows require `historical_dataset_stale` and `stale_at`.

`historical_analysis_dataset_candles` is an immutable full-value snapshot of each selected current canonical candle. It stores the zero-based contiguous position, source candle UUID/revision, warm-up flag, timeframe and UTC boundaries, complete OHLCV/trade values, source kind/counts/fingerprint, and creation time. It has CASCADE deletion from the dataset and `ON DELETE RESTRICT` to `market_candles`, unique `(dataset_id, position)` and `(dataset_id, candle_id)`, and `(dataset_id, open_time)` plus `(candle_id, dataset_id)` indexes. The preparation service validates exact UTC continuity and counts before insertion; the worker passes equivalent immutable snapshot inputs to the pure engine rather than reading mutable current candle rows during calculation.

`historical_analysis_reports` stores one immutable successful result per run and dataset. It contains UUID foreign keys to the run, owner, and dataset; unique `run_id` and `dataset_id`; lowercase SHA-256 result, dataset, and strategy fingerprints; immutable strategy and exit-reason-count JSONB snapshots copied at publication; engine, assumption, and calculation versions; schema-versioned JSONB market, preset, coverage, assumptions, and safety-disclosure snapshots; UTC analysis bounds; nonnegative signal/trade/equity counts; `trade_count`, `closed_trade_count`, and `open_at_end_count`; the count of signals without a valid next entry candle; exact-decimal summary metrics; and nullable win-rate/profit-factor values paired with an explicit undefined reason. Exit-reason counts are computed from the complete immutable closed-trade result before publication and must sum to `closed_trade_count`; they are not derived from paginated API rows. Run and owner deletion cascades; dataset deletion is restricted. Fingerprint, metric, range, count, and undefined-value checks enforce the immutable report contract. Legacy reports are backfilled with zero open-at-end and entry-unavailable counts.

`historical_analysis_trades` stores the complete immutable engine trade output for a report, including `trade_status` (`closed` or `open_at_end`), signal/position direction, candle identity/revisions/times, raw/fill prices, holding count, fee/slippage, returns, PnL, equity before/after, and outcome. The `outcome` column is nullable because an open-at-end position has unrealized rather than realized performance. Closed rows require exit candle/prices, realized outcome, exit reason, exit price basis, and exit-rule snapshot. Open-at-end rows require a final mark candle/revision/time/price and unrealized return/PnL, have no exit fields or outcome, and use their marked equity in `equity_after`; they never receive a fabricated max-holding exit. Nullable JSON fields for open-at-end rows are persisted as SQL `NULL`, not JSON `null`, so the database checks distinguish absent exit metadata from a populated snapshot. Legacy rows are backfilled as closed fixed-horizon six-candle, confirmed-close trades. It has CASCADE deletion from the report, `ON DELETE RESTRICT` candle references, unique `(report_id, sequence)`, and report/sequence plus candle-reference indexes. `historical_analysis_equity_points` stores one ordered immutable point per visible analysis candle with candle identity/revision/times, equity, drawdown, position state, and optional active trade sequence; the final point includes the open position's mark-to-market value, and it has the same report cascade/candle restrict pattern and unique `(report_id, sequence)`.
...
Historical-analysis creation first reads exact canonical coverage through the internal coverage service/repository boundary and fails fast before creating a run when the warm-up plus requested range is not usable. It never waits for or triggers market-data backfill. Dataset preparation then locks the run, takes a short `FOR SHARE` snapshot lock over the selected current canonical candles, validates coverage, and inserts the dataset metadata and immutable candle rows in one transaction. It requests the expected candle count plus one as a bounded defense against unexpected rows, and typed coverage failures persist only failed dataset metadata. Current-source validation locks the dataset and referenced market-candle rows for the check and atomically marks a changed dataset stale; it never rebuilds the same run. Final report publication repeats that validation while holding the source-row share locks and inserts the report, trades, equity points, and succeeded transition atomically.
...
Canonical candle retention defaults to 765 days, preserving the 730-day product range, maximum warm-up, and operational buffer; processed Telegram updates default to 30 days; signal-event retention configuration defaults to 365 days but no automatic signal-event deletion process is implemented. Feed stream cursor rows are retained for 7 days and the API listener deletes at most 10,000 expired rows in one maintenance transaction, without deleting referenced signal events. Signal dispatch and preset-signal outbox rows have no automatic cleanup in this change; terminal state remains available for recovery diagnosis and is governed by account-deletion FKs and future operational policy. The explicit `analysis:cleanup` command deletes only terminal historical-analysis runs older than `HISTORICAL_ANALYSIS_RETENTION_DAYS`, oldest first, up to `HISTORICAL_ANALYSIS_CLEANUP_BATCH_SIZE`; cascading removes their datasets, snapshots, reports, trades, and equity points. It never deletes queued or running work. Price events, alert records, and account-deletion retention remain governed by their FKs and operational policy.

## Migration Inventory

`20260728_0001` users/auth sessions; `20260730_0002` Telegram persistence; `0003` notification outbox; `0004` supported market catalogue; `0005` price alerts/events; `0006` live market state; `0007` price-alert notification kind; `0008` canonical candles; `0009` candle operational state; `0010` signal presets/subscriptions; `20260731_0011` signal evaluation/events/invalidations; `20260802_0012` durable signal-feed stream events and existing-event replay rows; `20260802_0013` explicit signal Telegram-delivery preference and immutable subscription state history with baseline rows; `20260802_0014` signal Telegram dispatch rows and preset-signal outbox references/indexes; `20260803_0015` owner-scoped historical-analysis runs; `20260803_0016` canonical historical-analysis dataset manifests and immutable candle snapshots; `20260803_0017` immutable historical-analysis reports, trades, and equity points plus restart-safe run requeue lifecycle; `20260803_0018` permits complete REST kline rows without unavailable trade-ID fields; `20260809_0019` immutable historical-analysis strategy snapshots and fingerprints; `20260809_0020` trade exit metadata with legacy backfill; `20260809_0021` immutable report strategy and exit-reason metadata; `20260810_0022` open-at-end trade status, mark metadata, report counts, nullable exit fields, and lifecycle constraints; `20260810_0023` 730-day historical-analysis dataset bounds; `20260810_0024` resumable candle archive checkpoints; `20260810_0025` persisted market/timeframe coverage; `20260810_0026` shared Binance REST rate state; `20260810_0027` permits nullable outcomes for open-at-end historical-analysis trades. The current branch's migration chain head is `20260810_0027`.
...
Schema contents were read from the SQLAlchemy registry and Alembic chain. Local verification applied Alembic head `20260810_0027` to the existing PostgreSQL volume and persisted a fresh open-at-end report with SQL-null outcome and exit metadata; rollback, schema-diff, and production migration behavior remain unverified.
