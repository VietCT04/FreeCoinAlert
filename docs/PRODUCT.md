...
- Configure an analysis with server-controlled assumptions available from the Info dialog, review the request in a dialog, follow its lifecycle, and inspect the focused Overview, Hypothetical trades, and Methodology tabs without client-side calculation. Overview shows the signal/date context, KPI cards, a stored-candle price chart with hypothetical buy/sell markers, and a two-decimal equity progression chart.
