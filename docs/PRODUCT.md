...
- Use the authenticated `Historical analysis` section after `Preset signals` to select one available market, one fixed preset version, and a bounded UTC date range; review server-controlled assumptions, track the run, cancel queued/running work, and inspect the successful report accessibly.
