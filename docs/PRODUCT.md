...
- Configure an analysis through one guided page with an explicit entry, long direction, server-supported exit rules, UTC period, server-controlled execution assumptions, and review step. Optional take-profit, stop-loss, and RSI threshold-cross exits retain exact decimal strings; maximum holding is required. The browser performs lightweight shape validation, maps safe server field errors, and submits the complete immutable strategy without calculating indicators or simulation results. After creation, processing and run-history rows show the server-returned strategy summary, while the focused Overview, Hypothetical trades, and Methodology tabs present the historical report.
...
The described code is implemented on the documented repository snapshot. A maintainer-requested local startup pass verified the Compose dependency graph, migrations, Binance market initialization, API/web health, and worker startup; the latest full isolated E2E pass verified the covered browser, simulator-backed Telegram, recovery, and historical-analysis behavior. Real-provider delivery, maintenance, reset, and broader production behavior remain unverified.
