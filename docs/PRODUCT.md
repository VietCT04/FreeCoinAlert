...
The repository-owned browser suite covers the current authentication, dashboard, Telegram, one-time price-alert, and preset-signal journeys through the real browser/API/provider-simulator boundaries. The route/action matrix is [E2E_COVERAGE.md](E2E_COVERAGE.md); the suite is Implemented but Unverified.
