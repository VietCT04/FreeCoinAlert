...
Run snapshots contain only controlled market/preset/strategy/version/range metadata and safe lifecycle fields. The strategy snapshot contains canonical server-validated rule values and a SHA-256 fingerprint; it never contains user formulas or executable code. The response omits user IDs, lock IDs, raw errors, candle IDs, provider payloads, credentials, and report bodies. The separate worker reads only server-owned run/dataset/snapshot rows, never accepts user formulas or assumptions, and publishes reports only for the claimed owner-scoped run. Report/trade/equity reads first authorize the run through the authenticated session and return no user ID, provider payload, raw exception, or mutable live state. Run creation performs server-owned configuration, strategy, range, and exact canonical-coverage checks only; incomplete coverage fails with a stable range-unavailable category. It does not wait for or trigger backfill, call Binance, calculate indicators, simulate trades, or create signal/alert/delivery work.

The pure fixed-preset simulation engine accepts only the server-controlled versions, preset definitions, calculation versions, validated immutable dataset snapshots, and the matching `fixed_horizon_v1` or `fixed_horizon_v2` assumptions. The v1 path preserves legacy incomplete-window semantics; the v2 path records open-at-end positions with final-close mark-to-market values. It has no database, HTTP, worker, file, browser, environment, or provider access and does not accept user-authored formulas or parameters. Its result carries fixed safety disclosures, including that it is not financial advice or a prediction and that synthetic short results are not executable Binance Spot trades. The engine result contains no credentials, user identifiers, provider payloads, or delivery state.

The historical-analysis browser flow keeps strategy drafts, run, report, trade, equity, fingerprint, and idempotency data in React memory only. The strategy builder sends exact decimal strings and only server-supported rule types; lightweight client validation is guidance, while server validation and ownership remain authoritative. Safe field-level validation details are mapped to controls without rendering internal messages. Charts and tables render server-provided values without client-side calculation or persistence, and the methodology view exposes full fingerprints only as report data to the already-authorized owner. Sign-in and sign-up retain the existing server validation and session flow; the card-based presentation adds no recovery, social-auth, or browser-storage path.

## Input Validation and Abuse Controls

Pydantic forbids unknown fields in security-sensitive request bodies; UUIDs, exact decimals, catalogue availability, strategy rule types/ranges, and lifecycle transitions are validated server-side. Strategy validation errors expose only stable field paths and codes, never submitted values or internal exceptions. Authentication, alert, signal, Telegram, and historical-analysis actions use bounded 15-minute in-memory rate-limit buckets. Historical-analysis creates are limited to 10 per user and 30 per direct client IP, cancellations to 30 per user, and configuration/list/detail/report/trade/equity reads to 120 per user. Signal Telegram-delivery preference mutations are limited to 30 per user per 15 minutes. These controls are process-local, not distributed abuse protection.
...
The isolated E2E environment is a separate trust boundary. `.env.e2e` uses a dedicated `_e2e` database, fixed project/network/volumes, non-production credentials, and exact `provider-simulator` Telegram/Binance URLs. E2E provider and control services have no host ports, no Docker socket, no production secrets, and no normal-volume access. E2E seed, control, and worker-gate modules refuse to run outside E2E mode or against a database whose name does not end in `_e2e`; the public API does not expose their routes. The control service's historical scenarios, before-claim/after-claim gates, link expiry, pagination fixtures, and owner-scoped signal invalidation remain guarded internal setup, not public API behavior. When E2E mode is false, custom Telegram API, file, and public bot URLs are rejected. The simulator accepts only its control token on internal mutation routes and records deterministic fixtures rather than production identities. The Playwright runner accepts only the fixed Compose project and `.env.e2e`, uses argument arrays with `shell: false`, verifies provider hostnames before startup, and removes only its fixed project, volumes, and `artifacts/e2e` directory.
...
Rate limits are process-local. There is no documented production backup, account-deletion, distributed rate-limit, security-review, penetration-test, or real-provider verification outcome. The isolated E2E browser and bounded artifact-redaction paths are verified by the full suite, but that pass does not establish production security or deployment safety. Runtime CORS/cookie deployment values require an explicit operational review before exposure beyond the configured origin.

The E2E browser suite registers users and exercises authentication through the UI, uses random in-memory passwords derived per test, transfers only the authenticated session cookie into feature-test contexts, and does not log or assert raw passwords, cookies, CSRF tokens, provider payloads, or internal identifiers. Provider and fixture mutations stay on the isolated internal simulator/control network; the suite does not intercept public API requests or contact real Binance/Telegram endpoints. Accessibility attachments contain only bounded rule IDs, impact, help text/URL, and shortened target selectors; complete Axe HTML or page payloads are not attached. Journey coverage is [E2E_COVERAGE.md](E2E_COVERAGE.md) and is Implemented and Verified for the isolated environment.

## Verification Status

Security behavior was inspected statically from code and configuration. The full isolated E2E pass exercised browser authentication, ownership boundaries, simulator isolation, safe provider outcomes, accessibility scans, bounded artifacts, migrations, and teardown. No penetration test, real-provider request, production deployment review, or standalone runtime security verification was run.
