...
- Historical analysis: authenticated API routes validate a fixed market/preset/range contract and persist an owner-scoped immutable request snapshot and lifecycle row. The internal dataset service locks the run, reads bounded current canonical candles, validates coverage, snapshots values atomically, fingerprints the manifest, and later marks it stale when the source revision no longer matches. No route, worker, calculation, simulation, report process, or provider call is added by this dataset boundary.
...
There is no message broker, cache cluster, independent market-data service, notification service, historical-analysis worker, independent dataset service, report service, or shared-package runtime. Signal-feed delivery intentionally uses PostgreSQL `LISTEN`/`NOTIFY` and a bounded process-local connection manager; it does not add a broker or cross-process connection state. Historical-analysis run and dataset persistence remain part of the API/database boundary until later approved issues add the pure engine, worker, and report execution.
