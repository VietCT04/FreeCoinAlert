...
- Historical analysis: authenticated API routes validate a fixed market/preset/range contract, persist an owner-scoped immutable request snapshot and lifecycle row, and expose only safe run metadata. Creation performs no candle read, calculation, worker dispatch, or provider call. Future dataset preparation, simulation, execution, and report processes remain separate boundaries.
...
There is no message broker, cache cluster, independent market-data service, notification service, historical-analysis worker, dataset service, report service, or shared-package runtime. Signal-feed delivery intentionally uses PostgreSQL `LISTEN`/`NOTIFY` and a bounded process-local connection manager; it does not add a broker or cross-process connection state. Historical-analysis run persistence is part of the API/database boundary until later approved issues add dataset, engine, worker, and report execution.
