...
The isolated E2E overlay retains the real web/API/database and application workers while replacing only external Binance/Telegram endpoints and fixture preparation. It adds `provider-simulator`, `e2e-seed`, and internal `e2e-control`, disables normal candle bootstrap, and uses a dedicated project, network, and volumes. The seed runs after migration and catalogue initialization; the market stream and historical-analysis worker wait for deterministic seed completion. The simulator and control service have no host port, and the public API does not mount E2E control routes. The complete E2E service contract is owned by [TESTING.md](TESTING.md).
