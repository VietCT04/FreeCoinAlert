...
The API routes authenticate browser requests and delegate to domain services. PostgreSQL persists durable state, immutable event history, occurrence-time subscription state history, signal Telegram dispatch state, the signal-feed transport cursor, historical-analysis run state, and canonical historical-analysis dataset snapshots. The API process owns a dedicated PostgreSQL listener and bounded in-process SSE connection manager; the listener loads durable rows and fans out only active-subscription sequences to local browser connections. The market stream owns one global Binance Spot aggregate-trade and kline pipeline, canonical candle persistence, aggregation, reconciliation, price-alert evaluation, and preset-signal evaluation. The Telegram poller consumes bot updates idempotently. The signal Telegram dispatcher claims occurrence work, evaluates occurrence-time subscription state, and creates immutable-snapshot outbox jobs without contacting Telegram. The notification worker claims test, price-alert, and preset-signal jobs; it strictly parses preset snapshots, rechecks server-owned delivery state, formats plain-text messages, and calls Telegram only after those checks. Historical-analysis dataset preparation and the fixed-preset simulation engine are internal API-package library boundaries, not process entry points.
...
- Historical analysis: authenticated API routes validate a fixed market/preset/range contract and persist an owner-scoped immutable request snapshot and lifecycle row. The internal dataset service locks the run, reads bounded current canonical candles, validates coverage, snapshots values atomically, fingerprints the manifest, and later marks it stale when the source revision no longer matches. The pure simulation engine recalculates fixed-preset signals from validated immutable snapshot inputs and returns an in-memory deterministic result. No route or worker invokes the engine, no report is persisted, and no provider call is made.
...
Historical-analysis dataset preparation locks the run and selected canonical rows, then commits the dataset manifest and all candle snapshots atomically. Current-source validation locks the dataset and references for a bounded check and commits a stale transition when any revision, value, source, or current-row identity changes.
...
There is no message broker, cache cluster, independent market-data service, notification service, historical-analysis worker, independent dataset service, report service, or shared-package runtime. Signal-feed delivery intentionally uses PostgreSQL `LISTEN`/`NOTIFY` and a bounded process-local connection manager; it does not add a broker or cross-process connection state. Historical-analysis run, dataset persistence, and the pure simulation module remain inside the API package/database boundary until later approved issues add worker and report execution.
