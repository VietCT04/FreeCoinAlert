...
### Production and broader runtime verification remains limited

**Current fact:** A local full-stack startup/readiness and status pass has run, and the maintainer-requested full isolated `pnpm e2e` pass completed all 65 browser cases with no failures, skips, or timeouts. That prior pass exercised the repository-owned browser foundation, responsive dashboard shell, overview, core price-alert/preset-signal/Telegram workflows, guided historical-analysis flow, Recharts report presentation, card-based authentication, recovery, mobile, accessibility, signal stream, audio boundary, simulator-backed Telegram delivery, report-tab loading, worker gates, and safe teardown. The current 730-day range admission, archive/checksum/checkpoint backfill, shared REST budget, persisted coverage boundary, deterministic long-range seed, paused-readiness amendment, calculated dataset bounds, historical-analysis open-at-end simulation/report semantics, nullable trade exit presentation, candlestick and hypothetical-order marker changes are unverified. Real Binance/Telegram providers, maintenance, reset, deployment recovery, and broader production runtime behavior remain unverified.

**Risk/impact:** The isolated E2E result demonstrates the covered local contracts, but it does not establish real-provider correctness, production deployment safety, or the behavior of uncovered maintenance and recovery paths.

**Current mitigation:** Documentation labels real-provider and broader runtime gaps unverified and avoids delivery guarantees; the full wrapper checks Compose one-shot exit and health state before its readiness banner, keeps status/logs/shutdown/reset controls explicit, and protects reset with confirmation; Compose dependency conditions make initialization ordering explicit; browser signal, dashboard, price-alert, preset-signal, Telegram, authentication, and historical-analysis surfaces keep client presentation separate from provider work, live occurrences, and optional sound; the isolated E2E overlay uses a dedicated `_e2e` database, named resources, exact simulator URLs, guarded seed/control/gate modules, and no simulator/control host ports; the dependency-free runner validates fixed safety inputs, uses one worker/no retries, checks service states before browser startup, retains bounded redacted artifacts, and always tears down its own volumes; the feature suite records coverage in [E2E_COVERAGE.md](E2E_COVERAGE.md), uses normal UI/API ownership paths, and limits internal controls to provider simulation, link expiry, named real-worker historical scenarios, worker gates, and owner-scoped invalidation fixtures; accessibility attachments are bounded summaries rather than full page payloads; the shared UI foundation keeps focus, status, error, table, chart, dialog, switch, tab, theme, navigation, and safe-state patterns in repository-owned source.

**Follow-up:** Request separate real-provider, maintenance, reset, deployment-recovery, and broader runtime verification when the maintainer is ready.
