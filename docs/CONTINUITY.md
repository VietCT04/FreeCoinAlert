...
- The owner-scoped historical-analysis run API and lifecycle persistence are Implemented and Unverified; dataset preparation, deterministic simulation, worker execution, reports, cleanup, and browser analysis remain Planned or Not supported. See [BACKTESTING.md](BACKTESTING.md) and [API.md](API.md).
...
| Historical-analysis run API | Implemented | Unverified | Owner-scoped request/lifecycle API only; no dataset, simulation, worker, report, or browser analysis runtime exists. |
