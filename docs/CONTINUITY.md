...
| Historical-analysis run, dataset, worker, report, series, and browser flow | Implemented | Unverified | Existing owner-scoped lifecycle, canonical coverage validation, immutable snapshots, worker cancellation/recovery, report publication, owner-only series reads, explicit cleanup behavior, report tabs, Recharts/table presentation, responsive run history, and named real-worker scenarios passed; the current presentation update awaits a maintainer-requested pass. |
