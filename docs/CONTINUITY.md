...
- The authenticated historical-analysis run API, canonical dataset preparation, immutable candle snapshots, pure deterministic fixed-preset and configurable-exit simulation engines, bounded version-dispatching worker, immutable report persistence, owner-scoped report/trade/equity reads, explicit terminal-run cleanup, and authenticated browser Choose inputs/Run analysis/Results flow are Implemented. The existing fixed API/worker scenarios are Verified by the isolated E2E pass; configurable execution and the current candle/order chart presentation are Unverified until a maintainer-requested pass. The browser includes responsive previous-run selection, three focused report tabs, a server-owned Recharts equity preview, and a bounded server-owned OHLC preview with hypothetical trade markers without client-side calculations; full equity pagination remains an API capability and is not loaded by the report presentation. See [BACKTESTING.md](BACKTESTING.md), [DATABASE.md](DATABASE.md), and [MARKET_DATA.md](MARKET_DATA.md).
...
| Historical-analysis run, dataset, fixed/configurable worker, report, series, and browser flow | Implemented | Unverified | Existing owner-scoped lifecycle, canonical coverage validation, immutable snapshots, versioned engine dispatch, worker cancellation/recovery, report publication, exit metadata, owner-only series reads, explicit cleanup behavior, focused report tabs, equity preview, bounded OHLC preview, hypothetical order markers, responsive run history, and named fixed-engine real-worker scenarios passed; configurable execution and the current chart presentation await a maintainer-requested pass. |
