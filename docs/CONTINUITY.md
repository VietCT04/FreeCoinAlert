...
- The authenticated historical-analysis run API, canonical dataset preparation, immutable candle snapshots, pure deterministic fixed-preset simulation engine, bounded worker, immutable report persistence, owner-scoped report/trade/equity reads, and explicit terminal-run cleanup are Implemented and Unverified; browser analysis remains Planned. See [BACKTESTING.md](BACKTESTING.md), [DATABASE.md](DATABASE.md), and [MARKET_DATA.md](MARKET_DATA.md).
...
| Historical-analysis run, dataset, worker, report, and series boundaries | Implemented | Unverified | Owner-scoped lifecycle API, canonical coverage validation, immutable snapshots, restart-safe worker/recovery, report publication, owner-only series reads, and explicit cleanup; browser analysis remains Planned. |
