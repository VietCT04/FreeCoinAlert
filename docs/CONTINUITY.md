...
- Local setup and provider preflight are Implemented and Unverified: the dependency-free Node entry point preserves an existing `.env`, validates the strict local contract, checks Docker/Compose and loopback ports, and reports `market`/conditional `telegram` selection without starting services or contacting providers. See [OPERATIONS.md](OPERATIONS.md) and [SECURITY.md](SECURITY.md).
...
| Local setup and provider preflight | Implemented | Unverified | Missing-only `.env` copy, strict configuration/URL/boolean/range validation, Docker/Compose checks, existing-service-aware port checks, and conditional profile reporting; no setup/preflight pass was requested. |
