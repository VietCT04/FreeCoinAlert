...
The active preset card and the authenticated historical-report monitor card present this server-owned preference separately from subscription status, browser history, and browser sound. They use a visible On/Off or enable action, map readiness to Ready, Linking, Not connected, or Needs attention guidance, confirm enabling before sending the CSRF-protected request, apply only the successful response, and never enable delivery automatically when an entry subscription is created. Connection refresh, linking, recovery, and disconnect changes refresh the subscription state so readiness does not remain stale. The browser stores no Telegram preference, readiness, destination identifier, token, or provider payload.
