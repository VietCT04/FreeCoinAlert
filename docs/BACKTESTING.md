...
The authenticated `/historical-analysis` route renders a guided Configure, Processing, Results flow inside the responsive dashboard shell. It uses native credentialed Fetch with the existing CSRF token for configuration, owner-scoped run creation/list/detail/cancellation, report, trade, and equity endpoints. A review dialog creates one UUID idempotency key and reuses it for an ambiguous retry; no optimistic run is shown and the key is kept in memory only. Previous runs remain owner-scoped, with a fixed desktop side panel and a mobile Sheet that preserves selection and opaque cursors. Selected queued/running runs are polled every five seconds while the document is visible, with polling paused while hidden and resumed immediately when visible.

The browser formats server-provided decimal strings and timestamps for display, renders the server's at-most-200-point equity preview through the repository-owned shadcn ChartContainer/Recharts composition, and keeps an accessible exact-value preview table directly available. It loads full immutable trades/equity only when their report tabs are first activated, then preserves bounded opaque-cursor pages. The chart converts exact decimal strings to finite plotting numbers only; it does not recalculate, normalize, interpolate, or downsample the series. The browser does not calculate indicators, metrics, drawdown, equity, or trade outcomes. Reports are labelled historical hypothetical simulations and remain separate from signal occurrences, alerts, sound, Telegram delivery, provider calls, recommendations, and financial advice.
