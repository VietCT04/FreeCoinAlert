...
The approved E2E environment is separate from normal local development and is not started by `pnpm dev:all`. Its runner invokes:
...
`.env.e2e` fixes the project to `freecoinalert-e2e`, uses database `freecoinalert_e2e`, ports `3100`, `8100`, and `55432`, and supplies the exact internal Binance/Telegram simulator URLs. The overlay uses a dedicated network and named volumes, does not expose `provider-simulator` or `e2e-control` on the host, disables normal candle bootstrap, and keeps real API/web/database/worker processes. The effective services are `db`, `api-prepare`, `db-migrate`, `provider-simulator`, `market-catalog-init`, `e2e-seed`, `api`, `web`, `market-stream`, `telegram-updates`, `notification-worker`, `signal-telegram-dispatcher`, `historical-analysis-worker`, and `e2e-control`.

The runner must remove stale `freecoinalert-e2e` resources before startup and remove the dedicated volumes after the run. It must not read `.env`, reuse normal project resources, mount the Docker socket, or expose simulator/control ports. `e2e-seed` inserts fixed UTC/exact-decimal canonical history after migration and catalogue initialization; the market stream and historical worker wait for the seed. The simulator and E2E control/seed/gate modules are guarded by E2E mode and the `_e2e` database suffix. See [TESTING.md](TESTING.md) for the verification boundary and control contract.
