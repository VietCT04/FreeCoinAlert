...
`.env.e2e` fixes the project to `freecoinalert-e2e`, uses database `freecoinalert_e2e`, ports `3100`, `8100`, and `55432`, and supplies the exact internal Binance/Telegram simulator URLs. The overlay uses a dedicated network and named volumes, does not expose `provider-simulator` or `e2e-control` on the host, disables normal candle bootstrap, and keeps real API/web/database/worker processes. The candle backfill worker is running but paused behind an E2E-only gate, so readiness is independent from deep history. The web service explicitly uses its isolated `E2E_WEB_ORIGIN` as `SITE_URL` and sets `SEO_INDEXING_ENABLED=true` so the public metadata regression suite can inspect production-style output; this setting is limited to the non-routable E2E network. The browser container uses internal `web:3000` and `api:8000` addresses with the matching CORS origin; the host-facing values remain fixed for explicit local inspection. The effective application services are `db`, `api-prepare`, `db-migrate`, `provider-simulator`, `market-catalog-init`, `e2e-seed`, `candle-backfill-worker`, `api`, `web`, `market-stream`, `telegram-updates`, `notification-worker`, `signal-telegram-dispatcher`, `historical-analysis-worker`, and `e2e-control`; `e2e-tests` is run on demand.
...
The parser ignores blank lines and comments, accepts `KEY=VALUE` with optional matching single or double quotes, rejects malformed non-comment lines, performs no shell expansion or file execution, and lets process-environment values override the corresponding `.env` values. Required local setup keys are `POSTGRES_DB`, `POSTGRES_USER`, `POSTGRES_PASSWORD`, `WEB_PORT`, `API_PORT`, `POSTGRES_PORT`, `NEXT_PUBLIC_API_BASE_URL`, `WEB_ORIGIN`, `SESSION_COOKIE_SECURE`, `BINANCE_SPOT_BASE_URL`, `BINANCE_SPOT_WS_BASE_URL`, `LOCAL_ENABLE_TELEGRAM`, `CANDLE_BACKFILL_TARGET_DAYS`, `CANDLE_BACKFILL_MAINTENANCE_SECONDS`, `CANDLE_BACKFILL_CHUNK_PAUSE_SECONDS`, and `LOCAL_STARTUP_TIMEOUT_SECONDS`. Ports must be distinct integers from 1 through 65535; HTTP URLs use `http` or `https`; the Binance WebSocket URL uses `ws` or `wss`; booleans are exact lowercase `true` or `false`; the background candle target is at least 765 days, maintenance is 60 through 86,400 seconds, chunk pause is 0 through 60 seconds, and startup timeout is 60 through 7,200 seconds.
...
`pnpm dev:market` and `pnpm dev:telegram` remain narrower profile-debugging commands. The full wrapper resolves the same `market`, `telegram`, and available `historical-analysis` profiles automatically. The market catalogue, background archive backfill, and market stream contact Binance; the poller and notification worker contact Telegram when configured; the signal dispatcher and historical-analysis worker contact only PostgreSQL. Catalogue synchronization is one-shot, the backfill and profile workers are long-running, and explicit bootstrap/reconciliation remain bounded maintenance commands.
...
`api-prepare`, `db-migrate`, and `market-catalog-init` use `restart: "no"` through their one-shot lifecycle. A failed preparation or migration blocks dependent services; catalog failure blocks the market stream and backfill worker. The backfill worker is `restart: unless-stopped` and non-blocking for application readiness. Telegram and historical-analysis profiles remain optional in Compose, but `pnpm dev:all` enables Telegram only when configured and enables historical analysis when its profile is available, so each enabled required worker is part of that command's readiness result. PostgreSQL and dependency volumes remain persistent, and repeating startup does not remove volumes or create duplicate logical initialization rows. Archive checkpoints and canonical coverage make historical work resumable.
...
| `pnpm dev:candle-backfill` | Continue resumable 765-day public archive coverage in the background. | Binance public archive, bounded REST fallback | Long-running |
...
`pnpm dev:all` first runs `docker compose <profiles> up --build --detach --wait --wait-timeout <LOCAL_STARTUP_TIMEOUT_SECONDS>`, then reads `docker compose <profiles> ps --all --format json`. It requires `api-prepare`, `db-migrate`, and `market-catalog-init` to have exited with code 0. It requires healthy `db`, `api`, and `web` services plus a running `market-stream`; the market backfill worker is reported but does not block readiness while it is starting, retrying, or extending coverage. Enabled Telegram services (`telegram-updates`, `notification-worker`, and `signal-telegram-dispatcher`) must be running. An available historical-analysis profile is enabled automatically and its worker must be running; when that profile is absent, the summary reports the worker as `unavailable`. After readiness succeeds, foreground mode runs `docker compose <profiles> up --watch` against the already-started project. Detached mode stops after the readiness summary and does not start a watcher.
...
`NEXT_PUBLIC_API_BASE_URL` is the browser-visible API origin and must contain no secret. `SITE_URL`, `SEO_INDEXING_ENABLED`, and optional `GOOGLE_SITE_VERIFICATION` are server-owned web settings described in [SEO.md](SEO.md); local and preview indexing stays disabled unless deliberately overridden. The authenticated dashboard shell and overview need no separate process, provider credential, or runtime configuration beyond the existing API origin and credentialed CORS/SSE proxy requirements. The overview performs a single explicit read-only refresh on mount and on user request; it does not poll or open SSE. Price-alert, preset-signal, and Telegram workflow pages use only their existing authenticated API clients, CSRF token, and current browser lifecycle settings; the Telegram usage summary performs two existing reads and has no separate worker or schedule. The browser signal and historical-analysis sections need no separate process, provider credential, audio asset, or runtime configuration beyond the same existing settings; historical analysis uses the Recharts and Lightweight Charts web dependencies and adds no new operational boundary. Historical-analysis browser controls use the existing authenticated API and CSRF token; they do not contact Binance or Telegram from the browser or require provider configuration in the web app.
...
`market-catalog-init` is the one-shot `market`-profile prerequisite for the stream and background candle worker. It calls Binance only for the fixed catalog, preserves the last valid metadata on failure, and returns non-zero so dependent market processes do not start. `market:sync` remains the equivalent direct operator command. Catalog synchronization is not scheduled by a separate service.

## Market Stream

Run one `market-stream` owner only. The Compose `market` profile starts it after successful catalogue synchronization; deep historical backfill is a separate non-blocking worker. It owns advisory lock `freecoinalert:market-stream:binance:spot`, refreshes catalog, reconnects with bounded backoff, and performs its implemented recent reconciliation. Two processes do not horizontally share stream ownership.

The stream's live candle write path is a closed-kline path: each accepted Binance `@kline_1m` event with `x=true` is sent to the candle ingestion service, while open kline updates and aggregate trades are not written to `market_candles`. The service performs an idempotent `1m` upsert, retains a new revision when confirmed values change, and rebuilds the affected `1h`/`4h` windows from current `1m` rows. The independent candle backfill worker imports validated public archives through the same ingestion service and therefore rebuilds affected derived windows without running signal delivery. See [MARKET_DATA.md](MARKET_DATA.md) for the persistence and derived-window contract.

## Candle Backfill, Bootstrap, and Reconciliation

Use the background candle backfill worker for deep historical coverage, `market:candles-bootstrap` for explicit bounded REST initialization, and reconciliation for missing recent data. The worker uses the public archive base URL, checksum/checkpoint state, monthly-to-daily fallback, bounded import chunks, and the dedicated backfill advisory lock. It never blocks API/web readiness and never waits for a user backtest. The running stream also requests a six-hour recent reconciliation during initial setup before its first WebSocket connection, then schedules it no more frequently than `CANDLE_RECENT_RECONCILIATION_SECONDS` (900 seconds by default). `market:candles-reconcile` is the explicit bounded command with a maximum of 168 hours. REST paths page requests at 1,000 minutes; direct recent commands acquire the market singleton lock, while the stream uses the lock it already owns.
...
Use `pnpm dev:setup` once, then use `pnpm dev:all` for normal local startup. API startup itself does not contact providers or write `market_candles`; the enabled market profile does so through the separate catalogue, background backfill, stream, and reconciliation paths. Core API/web readiness does not wait for the 765-day target. Foreground `pnpm dev:all` attaches Compose Watch with application logs and shuts down the Compose project without deleting volumes on Ctrl+C or normal termination. Restarting market processing relies on persisted current state and idempotent repositories rather than replaying raw trade history.
...
An earlier maintainer-requested `pnpm dev:all:detached` pass verified local database health, migrations, catalogue synchronization, bounded candle bootstrap, API/web health, market-stream startup, and the historical-analysis worker; `pnpm dev:status` also reported the expected normalized states. An earlier full `pnpm e2e` pass verified the prior isolated E2E stack, Playwright workspace, browser feature journeys, simulator-backed Telegram processes, worker gates, safe artifacts, and teardown. The updated archive/backfill worker, coverage path, paused-readiness amendment, production provider operations, maintenance commands, reset, and deployment recovery remain unverified.
