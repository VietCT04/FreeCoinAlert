...
`NEXT_PUBLIC_API_BASE_URL` is the browser-visible API origin and must contain no secret. The authenticated dashboard shell and overview need no separate process, provider credential, or runtime configuration beyond the existing API origin and credentialed CORS/SSE proxy requirements. The overview performs a single explicit read-only refresh on mount and on user request; it does not poll or open SSE. Price-alert, preset-signal, and Telegram workflow pages use only their existing authenticated API clients, CSRF token, and current browser lifecycle settings; the Telegram usage summary performs two existing reads and has no separate worker or schedule. The browser signal and historical-analysis sections need no separate process, provider credential, audio asset, or runtime configuration beyond the same existing settings; historical analysis uses the Recharts and Lightweight Charts web dependencies and adds no new operational boundary. Historical-analysis browser controls use the existing authenticated API and CSRF token; they do not contact Binance or Telegram from the browser or require provider configuration in the web app.
