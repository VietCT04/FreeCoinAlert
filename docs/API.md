...
The authenticated browser `Historical analysis` section consumes these routes with native credentialed Fetch. It displays only server-returned market/preset/configuration and report values through a guided Configure, Processing, Results flow with responsive previous-run selection and report tabs. It uses the existing CSRF token for create/cancel mutations, keeps an idempotency key in memory for ambiguous create retries, polls selected queued/running details only while visible, and loads trade/equity pages through the documented opaque cursors when their tabs are first activated. Its Recharts preview is presentation-only and the browser does not calculate indicators or metrics, store report data, or contact Binance or Telegram. The repository E2E suite exercises the same contracts for named lifecycle, cancellation, failure, pagination, reload, and ownership-revalidation states. No HTTP contract changes are introduced by this presentation boundary.
...
These contracts were read from routes, schemas, services, and browser consumers. The latest full isolated E2E pass exercised the documented browser/API flows and passed all 65 cases; uncovered API paths and production-provider behavior remain outside that pass.
