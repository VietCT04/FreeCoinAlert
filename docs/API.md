...
| GET | `/historical-analyses/{id}/report` | yes / no | owner-scoped immutable report, equity preview, candle preview, and hypothetical trade markers |
...
The authenticated dashboard browser surface consumes these contracts with native credentialed Fetch and `EventSource`. The `/dashboard` overview uses independent read-only calls to `/alerts?status=active&limit=20`, `/signal-subscriptions`, `/telegram/connection`, `/markets`, `/alerts?status=triggered&limit=5`, and `/signal-feed?status=all&limit=5`; it counts only returned owner-scoped rows, labels `/markets` as catalogue readiness, shows partial failures, and does not poll or open SSE. The `/price-alerts` surface sends the selected status as the existing `/alerts` query parameter, preserves its opaque cursor for Load more, and uses the existing create/delete contracts from a dialog and confirmed card actions. The `/preset-signals` surface keeps the existing subscription and feed request ownership, applies market/timeframe/subscription display filters in memory, and keeps both tab contents mounted while the route remains mounted. The `/telegram` usage summary repeats the existing active-alert and subscription reads with `Promise.allSettled`; it exposes counts or unavailable subsection states only and never changes the endpoint contracts. The signal feed keeps the historical pagination cursor separate from the `streamCursor` watermark, closes the stream when the document is hidden, refreshes history before visibility recovery, and treats replay or recovery entries as non-live UI updates. Active preset cards use the server-owned Telegram-delivery preference endpoint with the existing CSRF token, confirm enabling, apply only successful responses, and refresh subscriptions when their route is mounted. Browser sound is an optional client-side presentation feature and is not part of the API or Telegram delivery contract.

The current browser contract consumers are covered by the repository-owned E2E journeys for authentication, dashboard reads, Telegram linking/test/disconnect, price-alert creation/listing/deletion, and preset subscription/feed interactions. They use normal authenticated requests and the internal provider simulator only; no API interception or direct database setup is part of the suite. See [E2E_COVERAGE.md](E2E_COVERAGE.md). The covered journeys are Implemented and Verified through the latest full isolated E2E pass; this does not verify every API path or a production provider.

## Historical Analysis Runs

The authenticated HTTP API exposes bounded owner-scoped historical-analysis requests, lifecycle metadata, coverage status, and report reads. Run creation/list/detail/cancel routes do not read candles, calculate indicators, contact Binance, create alerts or signals, or perform simulation. A separate database worker prepares canonical immutable datasets and invokes the pure engine; report publication is atomic and the report routes read only the owner's persisted result.
...
```json
{
  ...
  "maximumRangeDays": 90,
  "maximumActiveRuns": 2,
  "simulationVersion": "historical_fixed_preset_v2",
  "assumptionVersion": "fixed_horizon_v2",
  "assumptions": {
    ...
    "endOfRange": "open_at_end_mark_to_market"
  },
...
The configuration endpoint describes the current fixed simulation contract and the server-owned strategy capability boundary; it does not invoke the pure engine. Configurable strategies are available only through the server-published capability contract and are executed by the versioned historical-analysis worker path. New runs use `historical_fixed_preset_v2`/`fixed_horizon_v2` or `historical_configurable_exit_v2`/`configurable_exit_v2`; persisted v1 runs remain readable and are dispatched with their original semantics. A report is available only after the worker publishes a successful run.
...
Only controlled Binance Spot markets, active fixed preset code/version pairs, and preset timeframes `1h` and `4h` are accepted. The server resolves the calculation snapshot (`sma_close_v1` for SMA presets or `rsi_wilder_close_v1` for RSI presets). An omitted strategy creates a `legacy_fixed_horizon_v1` snapshot with the six-candle entry/exit rule; the new simulation version opens a valid next-candle entry even when the full holding window does not fit and records `open_at_end` when no exit occurs. An explicit strategy creates a `configurable_exit_v1` snapshot containing long direction and canonicalized supported exit rules; the worker dispatches new requests to `historical_configurable_exit_v2`, while persisted v1 runs retain `historical_configurable_exit_v1`. RSI exits are threshold crossings on confirmed closes, not simple “currently above/below” checks. Callers cannot submit formulas, parameters, fees, slippage, sizing, or engine versions. Dates must be timezone-aware UTC values aligned to the selected timeframe, use an inclusive start and exclusive end, span 7 through 730 days, and end no later than the latest fully closed timeframe boundary. Before a run is created, the server checks the exact canonical warm-up plus analysis range through the internal coverage service/repository boundary. Incomplete or unusable coverage returns `409 HISTORICAL_ANALYSIS_RANGE_UNAVAILABLE`; the request path never waits for background backfill or contacts Binance. Existing request and response shapes remain compatible, and older runs/reports are not recalculated.

`GET /historical-analysis/coverage` requires authentication and accepts `exchange`, `market_type`, `symbol`, `preset_code`, `preset_version`, returning the server-owned coverage state for that market/preset. The response includes `backfilling`, `ready`, `degraded`, `partial`, or `unavailable`, the first safe analysis start after warm-up, latest safe analysis end, available complete days, target range, coverage percentage, and verification time. It is a read-only snapshot; it never starts a provider request or shortens a user-selected range. The browser uses it to progressively enable 30D/90D/6M/1Y/2Y and custom choices as coverage grows, without restarting the app.
...
These contracts were read from routes, schemas, services, and browser consumers. A previous full isolated E2E pass covered the earlier report contract; the current open-at-end trade fields, summary counts, and updated historical-analysis consumers were not runtime-verified in this change. Uncovered API paths and production-provider behavior remain outside that pass.
