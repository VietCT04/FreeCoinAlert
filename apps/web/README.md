...
- `/historical-analysis` — a guided Entry, Exit plan, Period, Execution assumptions, Review and run flow with server-driven long-only strategy rules, responsive previous-run selection, three focused report tabs, a server-owned Recharts equity preview, and a bounded candlestick preview with hypothetical trade markers.
...
The historical-analysis section uses credentialed native Fetch and the existing CSRF token for bounded run creation and cancellation. It presents one guided Entry, Exit plan, Period, Execution assumptions, Review and run flow, keeps previous runs in the desktop side panel or mobile Sheet, loads the report and trade pages from the existing owner-scoped endpoints, and polls selected queued/running runs only while the document is visible. When the server advertises configurable strategies, the builder exposes only its supported long-only exit rules and limits, keeps decimal inputs as exact strings, maps safe validation details to controls, and submits the immutable strategy with the existing request identity. The report uses server-provided assumptions, metrics, exact values, timestamps, a two-decimal KPI presentation, a Recharts equity preview, and a TradingView Lightweight Charts candlestick preview with hypothetical buy/sell markers; the browser does not calculate indicators or metrics and does not load the full equity pages. Run IDs, strategy drafts, reports, trades, candle/equity previews, fingerprints, and idempotency keys remain in memory only. In Compose, the web service waits for the API health check and carries no API, database, Telegram, or provider credentials.
