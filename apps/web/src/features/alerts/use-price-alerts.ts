...
export function usePriceAlerts({ authStatus, csrfToken, refreshSession }: UsePriceAlertsOptions): PriceAlertsState {
  ...
  const refreshAlerts = useCallback(async () => {
    ...
    try {
      ...
      setAlerts((current) => mergeAlerts(current, alertResponse.alerts));
  ...
  }, [authStatus, handleError, nextCursor]);
  ...
  }, [csrfToken, handleError, isCreating]);
  ...
  }, [authStatus, handleError, refreshAlerts]);
