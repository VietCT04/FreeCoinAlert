    ...
    async def consume(self, key: str, *, limit: int) -> None:
        async with self._lock:
            ...
            if len(bucket.timestamps) >= limit:
                retry_after = max(1, int(WINDOW_SECONDS - (now - bucket.timestamps[0])) + 1)
                raise AlertError(
                    status_code=429,
                    code="ALERT_RATE_LIMITED",
                    message="Too many alert requests. Try again later.",
...
def create_user_key(user_id: str) -> str:
    return f"alert-create-user:{user_id}"


def create_ip_key(client_ip: str) -> str:
    return f"alert-create-ip:{client_ip}"


def delete_user_key(user_id: str) -> str:
    return f"alert-delete-user:{user_id}"
